...
% si propaga il modello nel tempo usando i dati di test, ricostruendo  
% le immagini previste dalla dinamica DMD e creando una gif animata 
Xdmd = Uhat'*Xtest(:, 1);
t_test = size(Xtest,2);
gif_filename = 'dmd_animation.gif'; %SALVARE
figure();
for j = 1 : t_test-1
    %disp(j)
    Xdmd = [Xdmd, approxA*Xdmd(:,j)+approxB*Utest(:, j)];
   
    img_dmd = reshape(pinv(Uhat')*Xdmd(:,end), height, width); % Xdmd(:,end),...
    img_dmd_resized = imresize(img_dmd, 5, 'nearest'); % No
    imshow(img_dmd_resized, []); % imshow(img_dmd,...)
    axis image off;
    drawnow;
...
% modello DMDc
E = mean(abs(Uhat'*Xtest - Xdmd),1);
figure();
plot(E); % SALVARE
title('Average reconstruction error');
saveas(gcf, 'Average Reconstruction Error.png');
